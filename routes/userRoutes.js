...
router.post(
  ...
  [
    ...
    check(
      "screenName",
      "You must include a screenName with at least 5 characters"
    ).isLength({ min: 5 }),
    check(
      "password",
    ...
    try {
      ...
      if (user) {
        return res
          ...
          .json({
            errors: [
              { msg: "An account is already associated with the email" },
            ],
          });
      }
        ...
        return res
          ...
          .json({
            errors: [
              {
                msg:
                  "That screen name is already taken. Please select another.",
              },
            ],
          });
      }
...
router.post(
  "/login",
  [
    check("email", "Please include your email or screenName to login")
      .not()
      .isEmpty(),
    check("password", "You must include your valid password to login")
      .not()
      .isEmpty(),
  ],
  async (req, res) => {
    ...
    try {
      const user = await User.findOne({
        where: {
          [Op.or]: [{ email: req.body.email }, { screenName: req.body.email }],
        },
      });
